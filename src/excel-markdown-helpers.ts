...
export function addMarkdownSyntax(rows: string[][], columnWidths: number[]) {
    return rows.map(function (row, rowIndex) {
        ...
        return "| " + row.map(function (column, index) {
            ...
            return column + Array(columnWidths[index] - byteLength(column) + 1).join(" ");
        }).join(" | ") + " |";
    });
}
...
export function columnWidth(rows: string[][], columnIndex: number): number {
    return Math.max.apply(null, rows.map(function (row) {
        return (row[columnIndex] && byteLength(row[columnIndex])) || 0;
    }))
}
...
export function byteLength(str: string): number {
    ...
    // JavaScript 文字列の長さ（バイト数）を取得する https://zukucode.com/2017/04/javascript-string-length.html
    var i = 0;
    var byteLength = 0;
